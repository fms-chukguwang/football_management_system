...
export const joinTeamHtml = (from: SendJoiningEmailDto, recipient: TeamModel, token: string) => {
    ...
    <form action="${process.env.SERVER_HOST}:${process.env.FRONT_PORT || 3001}/api/team/${recipient.id}/user/${from.id}/approve" method="POST">
    ...
    <form action=${process.env.SERVER_HOST}:${process.env.FRONT_PORT || 3001}/api/team/${recipient.id}/user/${from.id}/reject" method="POST">
    <input type="hidden" name="token" value="${token}">
